...
public class practice {
    ...
    private static final String URL = "jdbc:sqlserver://localhost:1433;DatabaseName=Recruitment";
    private static final String USERNAME = "LOG1";
            ...
            try {
                ...
                if (affectedRows > 0) {
                    ...
                    try{
                        ...
                        sql= "INSERT INTO Stu_per_infom (ID, Stu_Name, Stu_Sex,Stu_School,Stu_Academy,Stu_Major,Stu_Result) VALUES (?,?, ?, ?,?,?,?)";
    ...
    //在校生功能1——查看招聘信息
    public static void viewRecruitmentInformation(Connection conn) throws SQLException {
        ...
        System.out.println("请问您是否需要进行简历投递？");
        String is_Submit=sc.next();
        if(is_Submit.equals("Y")||is_Submit.equals("y")){
            submitResume();
        }
        rs.close();
        ps.close();
    ...
    //在校生功能2——查看面试通知
    public static void viewInterviewNotifications(Connection conn) throws SQLException {
        //创建一个Statement对象用于执行SQL查询
        //通过用户ID查询
        String sql="SELECT Stu_Result FROM Stu_per_infom WHERE ID=?";
    ...
    public static void releaseInterviewNotice (Connection conn) {
        if (null != conn) {
            try {
                ...
                rs = ps.executeQuery( );
                while (rs.next()) {
                    String resumeID = rs.getString("Del_ResumeID");
                    System.out.println("所查询到该简历编号为："+resumeID);
                    String sqlResume="SELECT *FROM Resume_info WHERE ID = ?";
                    PreparedStatement ps1 = conn.prepareStatement(sqlResume);
                    ps1.setString(1,resumeID);
                    ResultSet rs1=ps1.executeQuery();
                    while ((rs1.next())) {
                        String resumeInfo = rs1.getString("ID");
                        resumeInfo += "\t" + rs1.getString("Res_Name");
                        resumeInfo += "\t\t" + rs1.getString("Res_Age");
                        resumeInfo += "\t\t" + rs1.getString("Res_PhoneNumber");
                        resumeInfo += "\t\t" + rs1.getString("Res_Email");
                        resumeInfo += "\t\t" + rs1.getString("Res_Job");
                        resumeInfo += "\t\t" + rs1.getString("Res_Educational");
                        resumeInfo += "\t\t" + rs1.getString("Res_Practice");
                        resumeInfo += "\t\t" + rs1.getString("Res_Skills");
                        resumeInfo += "\t\t" + rs1.getString("Res_Evaluation");
                ...
                if (result.equals("Y") || result.equals("y")) {
                    ...
                    String id = sc.next();
                    String insertSql = "UPDATE Stu_per_infom SET Stu_Result= ? WHERE ID= ?";
    ...
    public static void editPersonalInformation(Connection conn){
        try{
            String sql= "INSERT INTO Stu_per_infom (ID, Stu_Name, Stu_Sex,Stu_School,Stu_Academy,Stu_Major,Stu_Result) VALUES (?,?, ?, ?,?,?,?)";
            ps=conn.prepareStatement(sql);
            System.out.println("请完善个人基本信息：");
            System.out.println("编号：");
            ps.setString(1,sc.next());
